...
func (*TokenSvc) MakeToken(user *models.User) (string, error) {
	...
	claims["expDate"] = time.Now().Add(time.Hour * 24).Format("2006-01-02 15:04:05")

	token, err := tk.SignedString([]byte(config.GetConfig().TOKENKEY))
	if err != nil {
		return "", err
	}
	return token, nil
...
func (*TokenSvc) ParseToken(token string) (*models.User, error) {
	tk, err := jwt.Parse(token, func(token *jwt.Token) (interface{}, error) {
		return []byte(config.GetConfig().TOKENKEY), nil
	})
	if err != nil {
